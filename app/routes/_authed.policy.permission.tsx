...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
  ...
  const policies = await listPolicyConfiguration(user.token, 'permission:create') // TODO: drop the :transfer

  return json({items: policies, count: policies.length})
}

export default function PermissionPolicyConfigurePage() {
  ...
  return (
    <PolicyConfiguration configuration={config} kind="Permissions" />
  )
}
